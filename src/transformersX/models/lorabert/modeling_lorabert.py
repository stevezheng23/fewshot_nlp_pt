...
from .configuration_lorabert import LoraBertConfig
from ..bert.modeling_bert import BertEmbeddings as LoraBertEmbeddings
from ..bert.modeling_bert import BertEncoder as LoraBertEncoder
from ..bert.modeling_bert import BertPooler as LoraBertPooler
...
_TOKENIZER_FOR_DOC = "LoraBertTokenizer"
...
def load_tf_weights_in_lorabert(model, config, tf_checkpoint_path):
...
class LoraBertPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = LoraBertConfig
    load_tf_weights = load_tf_weights_in_lorabert
...
@add_start_docstrings(
    "The bare LoraBert Model transformer outputting raw hidden-states without any specific head on top.",
    LORABERT_START_DOCSTRING,
)
class LoraBertModel(LoraBertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = LoraBertEmbeddings(config)
        self.encoder = LoraBertEncoder(config)

        self.pooler = LoraBertPooler(config) if add_pooling_layer else None
    ...
    @add_start_docstrings_to_model_forward(LORABERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
...
@add_start_docstrings(
    """
    LoraBert Model transformer with a sequence classification head on top (a linear layer on top of the pooled output).
    """,
    LORABERT_START_DOCSTRING,
)
class LoraBertForSequenceClassification(LoraBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config

        self.bert = LoraBertModel(config)
        classifier_dropout = (
...
class LoraBertForDualPassageEncoder(LoraBertPreTrainedModel):
    def __init__(self, config, cls_loss_wgt=None):
        ...
        self.bert = LoraBertModel(config)
        self.pooler = LoraBertPooler(config)
    ...
    @add_start_docstrings_to_model_forward(LORABERT_INPUTS_DOCSTRING.format("batch_size, 2, sequence_length"))
    @add_code_sample_docstrings(
        tokenizer_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
