...
from .configuration_cutoffroberta import CutoffRobertaConfig
...
class CutoffRobertaPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = CutoffRobertaConfig
...
CUTOFFROBERTA_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~transformers.CutoffRobertaConfig`): Model configuration class with all the parameters of the
            model. Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the :meth:`~transformers.PreTrainedModel.from_pretrained` method to load the model
            weights.
"""

CUTOFFROBERTA_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`({0})`):
            Indices of input sequence tokens in the vocabulary.

            Indices can be obtained using :class:`~transformers.CutoffRobertaTokenizer`. See
...
@add_start_docstrings(
    "The bare CutoffRoBERTa Model transformer outputting raw hidden-states without any specific head on top.",
    CUTOFFROBERTA_START_DOCSTRING,
)
class CutoffRobertaModel(CutoffRobertaPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = CutoffRobertaEmbeddings(config)
        self.encoder = CutoffRobertaEncoder(config)

        self.pooler = CutoffRobertaPooler(config) if add_pooling_layer else None
...
class CutoffRobertaForSequenceClassification(CutoffRobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
