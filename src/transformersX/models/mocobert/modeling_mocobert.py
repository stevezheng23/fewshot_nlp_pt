...
class MoCoBertForDualPassageEncoder(MoCoBertPreTrainedModel):
    ...
    def forward(
        ...
        pooled_output = self.pooler(outputs[0])
        pooled_output = F.normalize(pooled_output, dim=-1)
        src_pooled_output, trg_pooled_output = pooled_output.reshape(b, 2, self.config.hidden_size).chunk(2, dim=1)
        ...
        with torch.no_grad():
            ...
            mo_pooled_output = self.mo_pooler(mo_outputs[0])
            mo_pooled_output = F.normalize(mo_pooled_output, dim=-1)
            mo_labels = torch.cat([labels, labels], dim=0)
        ...
        self._update_memory_bank(mo_pooled_output, mo_labels)
