...
from .configuration_c2bert import C2BertConfig
from ..bert.modeling_bert import BertEmbeddings as C2BertEmbeddings
from ..bert.modeling_bert import BertEncoder as C2BertEncoder
from ..bert.modeling_bert import BertPooler as C2BertPooler


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "bert-base-uncased"
_CONFIG_FOR_DOC = "C2BertConfig"
_TOKENIZER_FOR_DOC = "C2BertTokenizer"
...
def load_tf_weights_in_c2bert(model, config, tf_checkpoint_path):
...
class C2BertPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = C2BertConfig
    load_tf_weights = load_tf_weights_in_c2bert
...
@add_start_docstrings(
    "The bare C2Bert Model transformer outputting raw hidden-states without any specific head on top.",
    C2BERT_START_DOCSTRING,
)
class C2BertModel(C2BertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = C2BertEmbeddings(config)
        self.encoder = C2BertEncoder(config)

        self.pooler = C2BertPooler(config) if add_pooling_layer else None
    ...
    @add_start_docstrings_to_model_forward(C2BERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
...
class C2BertForSequenceClassification(C2BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.masking_prob = config.c2_masking_prob
        self.cls_loss_wgt = config.c2_cls_loss_wgt
        self.cl_loss_wgt = config.c2_cl_loss_wgt
        self.config = config

        self.bert = C2BertModel(config)
        classifier_dropout = (
