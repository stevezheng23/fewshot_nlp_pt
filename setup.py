...
7. Check that everything looks correct by uploading the package to the pypi test server:
   ...
   pip install -i https://testpypi.python.org/pypi transformersExt
...
if stale_egg_info.exists():
    print(
        (
            "Warning: {} exists.\n\n"
            "If you recently updated transformersX to 3.0 or later, this is expected,\n"
            "but it may prevent transformersX from installing in editable mode.\n\n"
...
# 2. once modified, run: `make deps_table_update` to update src/transformersX/dependency_versions_table.py
_deps = [
    "Pillow",
    "black==21.4b0",
    "codecarbon==1.2.0",
...
# since we save this data in src/transformersX/dependency_versions_table.py it can be easily accessed from
# anywhere. If you need to quickly access the data from this table in a shell, you can do so easily with:
#
# python -c 'import sys; from transformersX.dependency_versions_table import deps; \
# print(" ".join([ deps[x] for x in sys.argv[1:]]))' tokenizers datasets
#
# Just pass the desired package names to that script as it's shown with 2 packages above.
#
# If transformersX is not yet installed and the work is done from the cloned repo remember to add `PYTHONPATH=src` to the script above
#
# You can then feed this for example to `pip`:
#
# pip install -U $(python -c 'import sys; from transformersX.dependency_versions_table import deps; \
# print(" ".join([ deps[x] for x in sys.argv[1:]]))' tokenizers datasets)
#
...
class DepsTableUpdateCommand(Command):
    ...
    user_options = [
        ...
        ("dep-table-update", None, "updates src/transformersX/dependency_versions_table.py"),
    ]
    ...
    def run(self):
        ...
        target = "src/transformersX/dependency_versions_table.py"
...
# when modifying the following list, make sure to update src/transformersX/dependency_versions_check.py
install_requires = [
...
setup(
    ...
    license="Apache",
    url="https://github.com/huggingface/transformersX",
    package_dir={"": "src"},
    packages=find_packages("src"),
    extras_require=extras,
    entry_points={"console_scripts": ["transformers-cli=transformersX.commands.transformers_cli:main"]},
